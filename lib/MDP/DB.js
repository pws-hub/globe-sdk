...
function Query( table, dbClient ){
  ...
  this.insert = async ( data, operators = {} ) => {
    ...
    try {
      ...
      if( returnId ){
        const { _id } = await self.DBCollection.findOne( data, { _id: 1 } )
        sResponse.result = _id
      }

      if( result && result.ok == 1 ) return sResponse
  ...
  this.update = async ( conditions, data, operators = {}, target ) => {
    ...
    try {
      ...
      if( upsert || returnUpdate ){
        ...
        query = await self.DBCollection[ fn ]( conditions, toUpdate, superOps )
      }
      else query = await self.DBCollection[ fn ]( conditions, toUpdate )

      if( query.result && query.result.ok == 1 )
        return !query.result.nModified ? 'Already Up-to-date' : 'Updated'
    ...
    */
    ...
    try {
      ...
      if( result && result.ok == 1 )
        return !result.n ? 'Nothing to delete' : 'Deleted'
      else throw new Error('Not Found')
    }
    catch( error ){
      console.error( 'Find Query: ', error )
