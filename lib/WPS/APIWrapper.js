...
function To( verb, method, body ){
  return new Promise( ( resolve, reject ) => {
    ...
    if( typeof Globe_WPSConfig !== 'object' )
      return reject({ error: true, status: 'WPS_REQUEST::FAILED', message: 'Undefined Configuration' })

    const { server, headers } = Globe_WPSConfig
    
    request(`${server}/v1/${verb}`, { headers, method, form: body, json: true },
            ( error, resp, body ) => {
              if( error ) return reject({ error: true, status: 'WPS_REQUEST::FAILED', message: error })
              resolve( body )
            } )
  } )
}
