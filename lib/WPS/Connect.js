...
Manager = {
  emit: ( _event, payload ) => {
    ...
    if( typeof _event !== 'string' || !payload )
      throw new Error('Invalid Event Parameters')

    sendEvent( global.Globe_WPSConfig.host, _event, payload )
  },
...
*/
sendEvent = ( ...args ) => TransportReady && IOClient.emit( 'TRANSPORT::OUTGOING', ...args ),
