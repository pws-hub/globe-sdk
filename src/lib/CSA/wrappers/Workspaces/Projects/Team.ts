...
export default ( wid: string, to: any ) => {
  return ( pid: string ) => {
    const trunc = `workspaces/${wid}/projects/${pid}/members`
    ...
    async function members( limit: number = 20, offset?: number ){
      try { return await to( `${trunc}?limit=${limit}${offset ? '&offset='+ offset : ''}`, 'GET' ) }
      catch( error ){ return error }
    }

    async function member( mid: string ){
    ...
    return { add, members, member, update, remove, leave }
  }
}